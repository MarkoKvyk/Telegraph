...
using Telegraph.Contracts;
using Telegraph.Exceptions;
using Telegraph.Models;
using Telegraph.Models.Requests;
...
public class TelegraphClient : ITelegraphClient
{
	...
	/// <summary>
	///  Initializes a new instance of the <see cref="TelegraphClient" /> class.
	/// </summary>
	...
	public async Task<Page> CreatePage(string title, List<Node> content, string authorName = null,
	...
	/// <param name="authorUrl">Profile link, opened when users click on the author's name below the title. Can be any link, not necessarily to a Telegram profile or channel.</param>
	/// <returns>On success, returns a Page object.</returns>
	public async Task<Page> EditPage(string path, string title, List<Node> content, string authorName = null,
		string authorUrl = null)
	{
		ValidateAccessToken();
		ValidateStringParametr(title, nameof(title));
	...
	private async Task<PageViews> GetViewsGeneral(string path, int? year = null, int? month = null, int? day = null,
		int? hour = null)
	{
		ValidateStringParametr(path, nameof(path));
	...
	public async Task<List<TelegraphFile>> UploadFiles(IEnumerable<FileToUpload> files)
	{
		...
		foreach (var item in files)
		{
			if (!availableTypes.Contains(item.Type))
				throw new ArgumentException(
					$"MIME type of file {index} is not available. Available types: {string.Join(", ", availableTypes)}");
		...
		if (result.Type != JTokenType.Array) return telegraphFiles;
	...
	private void ValidateStringParametr(string parameter, string name)
	{
		if (parameter == null)
		...
		if (name == "title" && parameter.Length > 256)
			throw new ArgumentException("Should be less then 256 characters", name);
